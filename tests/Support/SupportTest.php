...
it('can get path', function () {
    ...
    expect($pathGenerator->getPath($media))->toBe('123');
});
...
it('can get path for conversions', function () {
    ...
    expect($pathGenerator->getPathForConversions($media))->toBe('123/conversions');
});
...
it('can get path for responsive images', function () {
    ...
    expect($pathGenerator->getPathForResponsiveImages($media))->toBe('123/responsive-images');
});
...
it('can apply prefix to path', function () {
    ...
    expect($pathGenerator->getPath($media))->toBe('my-prefix123');
});
